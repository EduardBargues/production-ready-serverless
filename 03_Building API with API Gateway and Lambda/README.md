...
2. Under `custom`, replace `<YOUR_NAME_HERE>` with your name (**no whitespaces, all lower case**), e.g. `ebargues`. This is so that if your project doesn't clash with your colleague's in case you're using a shared AWS account.
...
If you look into the DynamoDB console in your AWS account, you will see that the table would have an auto-generated name like `workshop-ebargues-dev-RestaurantsTable-1867PM9U2N4ZA`.

At this point, it's also worth peeking into the CloudFormation console, and see that the Serverless framework generated a CloudFormation stack for our project, and the name of the stack is derived from the `service` and `stage` name of our project - e.g. `workshop-ebargues-dev`.
...
**IMPORTANT**: `plugins` should be at the same level as `provider`, `functions` and `resources`, that is, it should have _NO INDENTATION_.
...
`restaurants_table=workshop-ebargues-dev-RestaurantsTable-1Y097GF7QLUIX`
      ...
      <ul class="column-container">
        ...
        <li class="item">
          <input
            ...
            type="text"
            size="50"
            placeholder="enter a theme, eg. cartoon"
          />
          <button onclick="search()">Find Restaurants</button>
        </li>
        <li>
...
- `${AWS::Region}`: this references the `AWS::Region` pseudo parameter, that is, the AWS region (e.g. `eu-west-1`) that you're deploying the CloudFormation stack to.
